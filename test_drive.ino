...
//후진 코드
// if문이므로 계속 써줘도 됨
//베이스 코드로 쓰려면 while문으로 수정, 부호 반대로
void Backward()
{
  if ( (Diff(FC_TRIG, FC_ECHO) < 5 ) && (Diff(L_TRIG, L_ECHO) < 5) && ( Diff(R_TRIG, L_ECHO) < 5 ) && (Diff(FL_TRIG, FL_TRIG) < 5) && ( Diff(FR_TRIG, FR_ECHO) < 5) )
  {
    Stop();
    SetSpeed(0);
  }
...
void AutoDriving()
{
  ...
  */
  ...
  if (cur_speed == 0) {                                    //정지상태에서 출발시
    ...
    f_center = GetDistance(FC_TRIG, FC_ECHO);
    f_left = GetDistance(L_TRIG, L_ECHO);
    f_right = GetDistance(R_TRIG, R_ECHO);
